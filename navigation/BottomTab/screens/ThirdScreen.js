...
const ThirdScreen = () => {
  ...
  // daty z wydarzeniem są zaznaczone
  const markedDate = {};
  ...
  // ładowanie wydarzeń z AsyncStorage
  ...
  // zapisywanie wydarzeń do AsyncStorage
  const saveEventsData = async (eventsData) => {
    try {
      await AsyncStorage.setItem('eventsData', JSON.stringify(eventsData));
    } catch (error) {
      console.error('Błąd podczas zapisywania danych do AsyncStorage:', error);
    }
  };
